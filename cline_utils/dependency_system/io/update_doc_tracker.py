...
def doc_file_inclusion_logic(
    ...
    abs_doc_roots: List[str] = [
        normalize_path(os.path.join(project_root, p)) for p in doc_directories_rel
    ]
    ...
    for norm_path, key_info in path_to_key_info.items():
        ...
        if any(
            norm_path == doc_root or is_subpath(norm_path, doc_root)
            for doc_root in abs_doc_roots
        ):
